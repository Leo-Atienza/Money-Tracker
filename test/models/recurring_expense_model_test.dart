  ...
  group('RecurringExpense', () {
    ...
    group('toMap()', () {
      ...
      test('serializes null maxOccurrences', () {
        final e = make(maxOccurrences: null);
    ...
    group('shouldBeActive', () {
      ...
      test('returns true when endDate is today', () {
        ...
        final e = make(isActive: true, endDate: today);
        expect(e.shouldBeActive, true);
      });
      ...
      test('returns true when maxOccurrences is null (unlimited)', () {
        final e = make(isActive: true, maxOccurrences: null);
        expect(e.shouldBeActive, true);
      });

      test('returns false when inactive even with future endDate', () {
        final futureDate = DateHelper.today().add(const Duration(days: 30));
        final e = make(isActive: false, endDate: futureDate);
        expect(e.shouldBeActive, false);
      });
    });
    ...
    group('tryFromMap()', () {
      test('parses a valid row', () {
        final parsed = RecurringExpense.tryFromMap(make(id: 7).toMap());
        expect(parsed, isNotNull);
        expect(parsed!.id, 7);
        expect(parsed.description, 'Netflix');
        expect(parsed.amount, closeTo(15.99, 0.001));
      });
      ...
      test('returns null on a row missing a required field', () {
        ...
        expect(RecurringExpense.tryFromMap(<String, dynamic>{}), isNull);
        expect(
          RecurringExpense.tryFromMap(make().toMap()..remove('description')),
          isNull,
        );
        expect(
          RecurringExpense.tryFromMap(make().toMap()..remove('account_id')),
          isNull,
        );
      });
      ...
      test('returns null on a wrong-typed column', () {
        expect(
          RecurringExpense.tryFromMap(make().toMap()..['dayOfMonth'] = 'x'),
          isNull,
        );
        expect(
          RecurringExpense.tryFromMap(make().toMap()..['account_id'] = 'x'),
          isNull,
        );
      });
      ...
      test('raw fromMap still throws on a corrupt row', () {
        expect(
          () => RecurringExpense.fromMap(<String, dynamic>{}),
