...
- [x] **5.5 Add Transaction (STRUCTURAL: replaces hub + merges add_expense/add_income)** — `feat(phase-5.5)` (session 8): new `lib/screens/add_transaction_screen.dart` replaces the three legacy screens with a single Luminous-styled form driven by a `GlassSegmentedControl<TransactionType>` toggle. Header is `GlassTopAppBar` with leading slot conditional on `Navigator.canPop` (BackButton when pushed, no leading when embedded as the center nav tab). `CategoryBentoGrid` is the new category picker; `GlassPanel` wraps every form section. Form-state contract preserves shared scalars across the type toggle (amount/description/date/tags/paymentMethod) and one-way-clears `_amountPaid` when toggling to Income (R15 mitigation, documented in `_onTypeChanged`). `OnboardingService.hasSeenAddTransactionTooltip` gates a one-time coach mark over the segmented control (R4 mitigation). Save flow keeps every behaviour from the legacy screens: future-date confirmation, expense-only budget warning, archived-category placeholder when editing a transaction whose category was deleted, atomic carryover via the existing `addExpense` / `addIncome` paths, cross-month "saved to X" snackbar, "Mark as Fully Paid" shortcut for unpaid expense edits, inline create-category / create-tag dialogs, PopScope unsaved-changes guard, payment-method chips, partial-payment validator. 7 caller sites updated (`main.dart` center tab; `home_screen.dart` empty-state push + long-press edit push; `history_screen.dart` modal-sheet edit for expense + push edit for income + two empty-state pushes). Old `add_hub_screen.dart` / `add_expense_screen.dart` / `add_income_screen.dart` moved to `TRASH/`. 5 widget tests in `test/screens/add_transaction_screen_test.dart` (renders contract, R15 toggle preservation, amount-paid one-way clear, first-launch tooltip with SharedPreferences persistence, edit mode hides segmented control). End-to-end submission is exercised at the AppState layer by `test/integration/app_state_crud_test.dart`.
- [x] **5.6 Transaction History split into `lib/screens/history/`** — `history_screen.dart` (2,306 lines) decomposed into 4 commits (session 7):
    - `phase-5.6.1` (`6bad3c3`): pure functions `groupByDay` / `groupByCategory` / `sortGroupKeys` / `formatDateHeader*` extracted to `history/history_grouping.dart` with 15 unit tests.
    - `phase-5.6.2` (`311c374`): `HistoryFilterBar` widget extracted — dumb StatelessWidget that owns the search field + filter-chip strip; parent retains state + debounce.
    - `phase-5.6.3` (`3544eb1`): `HistoryList` + `HistoryEmptyState` widgets — RefreshIndicator + ListView.builder shell with tile-builder callbacks; tile rendering stays in parent (deferred extraction).
    - `phase-5.6.4` (`f5e12ae`): file relocated to `lib/screens/history/history_screen.dart`. Parent file shrank 2306 → 1831 lines.
- [x] **5.7 Recurring Items (STRUCTURAL: merge expenses + income)** — `feat(phase-5.7)` (`1445371`): existing `recurring_expenses_screen.dart` and `recurring_income_screen.dart` relocated to `lib/screens/recurring/` and refactored from full screens into embeddable `RecurringExpensesView` + `RecurringIncomeView` widgets with public `showAddRecurringExpenseDialog` / `showAddRecurringIncomeDialog` entry points. New `lib/screens/recurring_items_screen.dart` composes both inside `GlassTopAppBar('Recurring Items')` + `GlassSegmentedControl<String>`. FAB color + target dialog swap with selected tab. DB row IDs and notification ID ranges (10000–29999) untouched. 4 widget tests in `test/screens/recurring_items_screen_test.dart`. 4 callers updated (`main.dart`, `add_hub_screen.dart`, two settings_screen entries).
...
- [x] **6.3 Backup AES-GCM + passphrase (crypto layer + UX wiring)** — `lib/utils/backup_crypto.dart` wraps `package:cryptography ^2.7.0` for 256-bit AES-GCM + PBKDF2-HMAC-SHA256 @ 100k iterations. Produces v4 envelope `{version, encrypted, salt, iv, ciphertext, tag}`. `decrypt` returns null on any failure (wrong passphrase, malformed JSON, tampered ciphertext, wrong salt) — never throws, never silently returns wrong plaintext. **Session 4: UX wiring landed.** `BackupHelper.wrapBackupIfNeeded` / `unwrapBackupIfNeeded` are `@visibleForTesting` static methods that pair the crypto module with the existing comprehensive backup path. `saveBackupToUserSelectedLocation`, `shareDatabase`, and `restoreDatabase` each accept the passphrase contract. `backup_restore_screen.dart` shows a confirmation dialog with min-6-char passphrase before any save/share, and a retry-loop dialog when restoring an encrypted envelope (loops until correct passphrase or user cancels). Legacy v2/v3 plaintext backups still restore transparently — `BackupCrypto.isEncryptedEnvelope` guards the decrypt branch. 26 tests total (15 crypto + 11 helper integration). **Smoke test on device for the dialogs still required before tagging v5.0.0+1.**
...
- [x] **7.2 AppState mutator coverage** — new `test/logic/app_state_mutators_test.dart` covers the safe, DB-free subset (settings/appearance/filters): `setThemeMode` (3 cases), `toggleDarkMode`, `toggleShowTransactionColors`, `setTransactionColorIntensity` (clamping), `toggleBillReminders`, `toggleBudgetAlerts`, `toggleMonthlySummary`, `setReminderTime`, `setFilterCategory`, `setDateRange`, `setAmountRange`, `setPaidStatusFilter`, `clearFilters`, `clearAutoCreatedCount`. 22 tests. DB-touching mutators (addExpense/addIncome/setBudget/addAccount/etc.) deferred to integration files. (session 5 — `de32374`)
    - [x] **D.1 complete — DB-touching mutator coverage** — `test/integration/app_state_crud_test.dart`. Session 7 (`6b10877`) landed the first 16 tests for the add/delete/account/category/budget happy + validation paths. Session 8 appended +24 tests (40 pass + 3 env-skipped) covering `updateExpense` (description; date-move via `allExpenses` since the filtered getter scopes to selectedMonth), `updateIncome`, `addPayment` (full + sub-10c auto-round to paid), `restoreDeletedExpense` / `restoreDeletedIncome`, `permanentlyDeleteExpense` / `permanentlyDeleteIncome`, `emptyTrash`, recurring expense CRUD (add + update with notification-platform-mock try/catch + delete skipped pending mock), recurring income CRUD (mirrored), quick-template add / update / delete / useTemplate, `updateCategory` (rename back-propagates to expenses), `reassignCategoryAndDelete` + `deleteTransactionsAndCategory` (seed a non-default category because the bulk-delete SQL guards `isDefault = 0`), `updateAccount`, `switchAccount` (with `onAccountSwitch` stream drain), `setOverallMonthlyBudget` / `removeOverallMonthlyBudget` round-trip, and `restoreDeletedAccount` (skipped — `path_provider` mock races with orphaned-file cleanup under parallel flutter_test isolates).
