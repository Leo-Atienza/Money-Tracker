...
- [x] **7.2 AppState mutator coverage** — new `test/logic/app_state_mutators_test.dart` covers the safe, DB-free subset (settings/appearance/filters): `setThemeMode` (3 cases), `toggleDarkMode`, `toggleShowTransactionColors`, `setTransactionColorIntensity` (clamping), `toggleBillReminders`, `toggleBudgetAlerts`, `toggleMonthlySummary`, `setReminderTime`, `setFilterCategory`, `setDateRange`, `setAmountRange`, `setPaidStatusFilter`, `clearFilters`, `clearAutoCreatedCount`. 22 tests. DB-touching mutators (addExpense/addIncome/setBudget/addAccount/etc.) deferred to integration files. (session 5 — `de32374`)
    - [x] **D.1 partial — DB-touching mutator coverage** — `test/integration/app_state_crud_test.dart` (`6b10877`, session 7): 16 end-to-end tests covering `addExpense` (happy + 3 validation paths), `addIncome` (happy + zero validation), `deleteExpense` / `deleteIncome` (trash round-trip), `addAccount` (empty / whitespace rejection + append), `setDefaultAccount` (single-default invariant), `deleteAccount` (last-account refusal + delete non-current), `addCategory` + `deleteCategory` round-trip, `setBudget` (upsert + overwrite). Remaining gaps for full D.1: `updateExpense`, `updateIncome`, `markExpensePaid`, `restoreDeleted*`, `permanentlyDelete*`, `emptyTrash`, `clearOldDeleted`, `updateCategory`, `deleteTransactionsAndCategory`, `reassignCategoryAndDelete`, all recurring CRUD, template CRUD, `setAlertThreshold`.
